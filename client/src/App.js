      ...
      <div className="suggestions">
        ...
        {result.suggestions.map((suggestion, index) => (
          <div key={index} style={{display: 'none'}}>
            {suggestion.currentSituation && (
              <div className="current-situation-analysis">
                <h5>🔍 Nuvarande situation i din kod:</h5>
                <div className="situation-details">
                  <p><strong>Implementation:</strong> {suggestion.currentSituation.implementation}</p>
                  
                  {suggestion.currentSituation.whatYouDoNow && (
                    <div className="what-you-do-now">
                      <div dangerouslySetInnerHTML={{__html: suggestion.currentSituation.whatYouDoNow}} />
                    </div>
                  )}
                  
                  {suggestion.currentSituation.whatIsTheProblem && (
                    <div className="what-is-problem">
                      <div dangerouslySetInnerHTML={{__html: suggestion.currentSituation.whatIsTheProblem}} />
                    </div>
                  )}
                  
                  {suggestion.currentSituation.howOthersCanHelp && (
                    <div className="how-others-help">
                      <div dangerouslySetInnerHTML={{__html: suggestion.currentSituation.howOthersCanHelp}} />
                    </div>
                  )}
                  
                  {suggestion.currentSituation.detailedExplanation && (
                    <div className="detailed-explanation">
                      <strong>Sammanfattning:</strong>
                      <p>{suggestion.currentSituation.detailedExplanation}</p>
                    </div>
                  )}
                  
                  {suggestion.currentSituation.codeExamples && (
                    <div className="code-example">
                      <strong>Exempel från din kod:</strong>
                      <pre><code>{suggestion.currentSituation.codeExamples.join('\n')}</code></pre>
                    </div>
                  )}
                  
                  {suggestion.currentSituation.whatThisMeans && (
                    <div className="what-this-means">
                      <strong>Vad detta betyder för dig:</strong>
                      <p>{suggestion.currentSituation.whatThisMeans}</p>
                    </div>
                  )}
                  
                  <div className="performance-issues">
                    <strong>Identifierade flaskhalsar:</strong>
                    <ul>
                      {suggestion.currentSituation.performanceBottlenecks?.map((issue, i) => (
                        <li key={i}>{issue}</li>
                      ))}
                    </ul>
                  </div>
                  
                  {suggestion.currentSituation.measuredImpact && (
                    <div className="measured-impact">
                      <strong>Mätt prestanda:</strong>
                      <div className="metrics">
                        <span>Svarstid: {suggestion.currentSituation.measuredImpact.avgResponseTime}</span>
                        <span>Databasbelastning: {suggestion.currentSituation.measuredImpact.databaseLoad}</span>
                        <span>Skalbarhet: {suggestion.currentSituation.measuredImpact.scalabilityLimit}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {suggestion.afterImprovement && (
              <div className="after-improvement-analysis">
                <h5>🚀 Efter förbättring:</h5>
                <div className="improvement-details">
                  <p><strong>Ny implementation:</strong> {suggestion.afterImprovement.implementation}</p>
                  
                  {suggestion.afterImprovement.whatYouWillDoInstead && (
                    <div className="what-you-will-do">
                      <div dangerouslySetInnerHTML={{__html: suggestion.afterImprovement.whatYouWillDoInstead}} />
                    </div>
                  )}
                  
                  {suggestion.afterImprovement.whyThisIsBetter && (
                    <div className="why-this-better">
                      <div dangerouslySetInnerHTML={{__html: suggestion.afterImprovement.whyThisIsBetter}} />
                    </div>
                  )}
                  
                  {suggestion.afterImprovement.howMuchBetter && (
                    <div className="how-much-better">
                      <div dangerouslySetInnerHTML={{__html: suggestion.afterImprovement.howMuchBetter}} />
                    </div>
                  )}
                  
                  {suggestion.afterImprovement.detailedExplanation && (
                    <div className="detailed-explanation">
                      <strong>Sammanfattning:</strong>
                      <p>{suggestion.afterImprovement.detailedExplanation}</p>
                    </div>
                  )}
                  
                  {suggestion.afterImprovement.codeExamples && (
                    <div className="code-example improved">
                      <strong>Ny kod:</strong>
                      <pre><code>{suggestion.afterImprovement.codeExamples.join('\n')}</code></pre>
                    </div>
                  )}
                  
                  {suggestion.afterImprovement.whatThisMeans && (
                    <div className="what-this-means improved">
                      <strong>Vad detta betyder för dig:</strong>
                      <p>{suggestion.afterImprovement.whatThisMeans}</p>
                    </div>
                  )}
                  
                  <div className="performance-gains">
                    <strong>Prestandaförbättringar:</strong>
                    <ul>
                      {suggestion.afterImprovement.performanceGains?.map((gain, i) => (
                        <li key={i}>{gain}</li>
                      ))}
                    </ul>
                  </div>
                  
                  {suggestion.afterImprovement.measuredImpact && (
                    <div className="measured-impact improved">
                      <strong>Förväntad prestanda:</strong>
                      <div className="metrics">
                        <span>Svarstid: {suggestion.afterImprovement.measuredImpact.avgResponseTime}</span>
                        <span>Databasbelastning: {suggestion.afterImprovement.measuredImpact.databaseLoad}</span>
                        <span>Skalbarhet: {suggestion.afterImprovement.measuredImpact.scalabilityLimit}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="apis">
              <strong>Rekommenderade leverantörer:</strong>
              {suggestion.providers?.map((provider, i) => (
                <div key={i} className="provider-recommendation">
                  ...
                  <div className="provider-details">
                    ...
                    <span className="provider-roi">ROI: {provider.roi}</span>
                  </div>
                  <p className="provider-desc">{provider.description}</p>
                  <a href={provider.url} target="_blank" rel="noopener noreferrer" className="provider-link">
                    Läs mer →
                  </a>
                </div>
              )) || (
                <div className="no-providers">
                  <p>Inga specifika leverantörer hittades för detta förslag.</p>
                </div>
              )}
            </div>
            ...
            {suggestion.implementationSteps && (
              <div className="implementation-steps">
                <strong>Implementationssteg:</strong>
                <ol>
